...
namespace unnu
{
  ...
  { // anonymous/private namespace for assertions
    ...
    */
    template <typename F>
    void
    assert_arguments(const F generator, const std::size_t n, const Range range)
    {
      using generator_return_t = typename std::result_of<F()>::type;

      static_assert(std::is_integral<generator_return_t>::value,
                    "CAUSE: The \"generator\" function must return a integral "
                    "data type.");
  ...
  n_unique_fromOLD(F generator, const std::size_t n, Range range)
  {
    ...
    for (std::size_t i = 0; i < n; i++)
    {
      ...
      assert(inserted && "A duplicate was generated, verify if `generator` can "
  ...
  /*  n_unique_from
     ...
     to `generator`
      @param  `generator`:    function/functor that will generate values to be
     returned
  ...
  */
  ...
  template <typename F>
  std::set<typename std::result_of<F()>::type>
  n_unique_from(F generator, const std::size_t n, Range range)
  {
    assert_arguments(generator, n, range);
    ...
    std::size_t last_offset = 0; // no. elements smaller than last
    ...
    for (std::size_t i = 0; i < n; i++)
    {
      ...
      assert(gen.size() == i + 1 && "A duplicate was generated, verify if "
                                    "`generator` can generate enough numbers");
    }
  ...
  */
  template <typename F>
  std::set<typename std::result_of<F()>::type>
  n_unique_from(F generator, const std::size_t n, std::size_t max)
  {
    Range r = {0, max};
  ...
  template <typename F>
  std::set<typename std::result_of<F()>::type>
  n_unique_fromOLD(F generator, const std::size_t n, std::size_t max)
  {
    Range r = {0, max};
